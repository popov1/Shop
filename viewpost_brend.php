...
<?php
		...
		$sql = "SELECT price, name, size, id FROM tyre where brend like '%".$_SESSION['name']."%' ORDER BY id ASC";
		...
		$r = mysql_query("SELECT price, name, size, id FROM tyre where brend like '%".$_SESSION['name']."%' ORDER BY id ASC");
		$kol_row = mysql_num_rows($r);
		$r = mysql_query("SELECT price, name, size, id FROM tyre where brend like '%".$_SESSION['name']."%' ORDER BY id ASC LIMIT $start, $kol_per_page ");
		$n = mysql_num_rows($r); // возвращаем число рядов результата запроса	
		// если страница не первая, выводим ссылку НАЗАД
		if ($str > 0)
		{
		 $p = $str - 1;
		 echo "<a href=viewpost_brend.php?str=$p>НАЗАД</a>	&nbsp;	&nbsp;	&nbsp;";
		}
		...
		if($start + $n < $kol_row)
		{
		  echo "<a href=viewpost_brend.php?str=$str>ДАЛЕЕ</a>"; 
		} 
